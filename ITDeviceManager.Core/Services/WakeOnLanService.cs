...
public class WakeOnLanService : IWakeOnLanService
{
    ...
    public async Task<bool> WakeDeviceAsync(string macAddress, string? ipAddress = null)
    {
        try
        {
            _logger.LogInformation("[WOL] 开始唤醒设备 - MAC: {MacAddress}, IP: {IpAddress}", macAddress, ipAddress);

            var macBytes = ParseMacAddress(macAddress);
            if (macBytes == null)
            {
                _logger.LogWarning("[WOL] MAC地址解析失败: {MacAddress}", macAddress);
                return false;
            }

            _logger.LogInformation("[WOL] MAC地址解析成功: {MacBytes}", BitConverter.ToString(macBytes));

            var magicPacket = CreateMagicPacket(macBytes);
            _logger.LogInformation("[WOL] 魔术包创建完成，大小: {PacketSize} 字节", magicPacket.Length);

            using var client = new UdpClient();
            client.EnableBroadcast = true;

            // 发送到广播地址
            _logger.LogInformation("[WOL] 发送魔术包到广播地址 255.255.255.255:9");
            ...
            {
                _logger.LogInformation("[WOL] 发送魔术包到目标地址 {TargetIP}:9", targetIP);
                await client.SendAsync(magicPacket, magicPacket.Length, new IPEndPoint(targetIP, 9));
            }

            _logger.LogInformation("[WOL] Wake-on-LAN 包发送成功");
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[WOL] Wake-on-LAN 发送失败");
            return false;
        }
    }
    ...
    public Task<bool> ShutdownDeviceAsync(string ipAddress)
    {
        ...
        // 实际生产环境中需要使用WMI或其他方法
        _logger.LogWarning("远程关机功能需要额外配置，设备: {IpAddress}", ipAddress);
