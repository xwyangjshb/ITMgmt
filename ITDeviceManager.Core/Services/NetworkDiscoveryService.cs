...
public class NetworkDiscoveryService : INetworkDiscoveryService
{
    ...
    private async Task<DeviceType> IdentifyDeviceTypeAsync(string ipAddress, string macAddress, string? hostname)
    {
        try
        {
            ...
            // 添加调试日志
            _logger.LogDebug("[DEBUG] 识别设备类型 - IP: {IpAddress}, MAC: {MacAddress}, 前缀6: {MacPrefix6}, 前缀8: {MacPrefix8}", ipAddress, macAddress, macPrefix6, macPrefix8);
            ...
            {
                _logger.LogDebug("[DEBUG] 匹配到6位前缀: {MacPrefix6} -> {DeviceType}", macPrefix6, vendorPrefixes[macPrefix6]);
                return vendorPrefixes[macPrefix6];
            }

            if (vendorPrefixes.ContainsKey(macPrefix8))
            {
                _logger.LogDebug("[DEBUG] 匹配到8位前缀: {MacPrefix8} -> {DeviceType}", macPrefix8, vendorPrefixes[macPrefix8]);
                return vendorPrefixes[macPrefix8];
            }

            _logger.LogDebug("[DEBUG] 未匹配到MAC前缀，继续主机名识别");
            ...
            _logger.LogDebug("[DEBUG] 最终返回未知设备类型");
            return DeviceType.Unknown;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[DEBUG] 设备类型识别异常");
            return DeviceType.Unknown;
        }
    }
