...
public class ChinaDateTimeConverter : JsonConverter<DateTime>
{
    ...
    static ChinaDateTimeConverter()
    {
        ...
        catch
        {
            ...
            try
            {
                _chinaTimeZone = TimeZoneInfo.FindSystemTimeZoneById("Asia/Shanghai");
            }
            catch
            {
                // 如果都找不到，将使用固定偏移量
    ...
    {
        if (_chinaTimeZone != null)
        {
            // 使用系统时区转换
            return TimeZoneInfo.ConvertTimeFromUtc(utcDateTime, _chinaTimeZone);
        }
        else
        {
            // 使用固定偏移量 UTC+8
            return utcDateTime.Add(_chinaOffset);
        }
    }
}
